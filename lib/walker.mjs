...
export function walker(sel) {
  ...
  // display:none / visibility:hidden のテキストは運ばない（.claim-ref 等の非表示注釈）
  const isHidden = (el) => {
    const st = getComputedStyle(el)
    return st.display === 'none' || st.visibility === 'hidden'
  }
  const isSkipped = (el) =>
    isUiSkipped(el) || !!el.closest('[aria-hidden="true"]') || isHidden(el)
  const toHex = (rgb) => {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        const codeBg =
          codeEl && !codeEl.closest('pre')
            ...
            : null
        // ハイパーリンクは URL と下線を run に運ぶ（dev server 内部リンクは除外）
        const aEl = node.parentElement.closest('a[href]')
        const hrefUrl =
          aEl && /^https?:/.test(aEl.href) && !aEl.href.startsWith(location.origin)
            ? aEl.href
            : null
        runs.push({
          text,
  ...
  // 左縦線だけの装飾ボーダー（blockquote・quote レイアウト等）:
  // border-left のみ可視の要素は、その線を細い矩形として運ぶ。
  // 全周ボーダーの箱は対象外（テキストとして再構築される内容の背景装飾は
  // 現状スコープ外のため、引用バーのパターンに限定する）
  for (const el of root.querySelectorAll('*')) {
    if (isUiSkipped(el) || el.closest('[data-diag="root"], [data-diag="drawio"]')) continue
    const st = getComputedStyle(el)
    const lw = parseFloat(st.borderLeftWidth) || 0
    if (!lw || st.borderLeftStyle === 'none' || st.borderLeftStyle === 'hidden') continue
    const hasOther = ['Top', 'Right', 'Bottom'].some(
      (s) => (parseFloat(st[`border${s}Width`]) || 0) > 0,
    )
    if (hasOther) continue
    const fill = toHex(st.borderLeftColor)
    if (!fill) continue
    const r = el.getBoundingClientRect()
    if (r.width === 0 || r.height === 0) continue
    const rr = rel(r)
    elements.push({
      kind: 'diag-box',
      rect: { x: rr.x, y: rr.y, w: lw * scale, h: rr.h },
      fill,
      border: fill,
      borderWPx: 0,
      radiusPx: 0,
