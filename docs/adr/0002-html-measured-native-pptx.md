...
Slidev の公式 PPTX export は各スライドを画像としてページ背景に配置する方式で、テキストは編集できない（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）。一方で、顧客・社内共有の慣行上、PowerPoint で開いて手直しできる PPTX が求められる場面がある。Markdown（CSS レイアウト・リフロー前提）と PPTX（EMU 絶対座標・リフローなし）は構造的に不整合であり、ソースをテキストで保つ要件と PowerPoint 上の編集性は衝突する。
...
この思想は marp-to-editable-pptx（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）を継承する。Slidev では公式 export が画像ベースであるため、本リポジトリの `bin/slidev-editable-pptx.mjs` が同じ考え方を Slidev 用に実装する。
...
Marp CLI `--pptx-editable` は experimental で、PDF を LibreOffice Impress で PPTX に変換する二段経路に依存する（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）。複雑テーマでの失敗、再現性低下、発表者ノート非対応が公式に明記されており、CSS テーマは意味的に消滅し、日本語はフォント代替と文字単位の分断が起きる。

ppt-master は AI が描いた SVG を python-pptx で native DrawingML に変換し、編集可能な PPTX を生成する（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）。ただしソースは SVG 中間表現であり Markdown ではない。textlint、スライド本文の出典管理、git diff の意味性という本基盤の前提から外れる。
...
* Bad, because PDF→LibreOffice の二段変換であり、テーマ消滅、日本語分断、複雑テーマでの不完全出力がある（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）。
...
* Good, because SVG から native DrawingML へ変換し、真に編集可能な PPTX を生成する（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）。
* Bad, because ソースが SVG であり Markdown ではない。対話セッション前提で CI に載せにくく、textlint とスライド本文の出典管理の経路が切れる。
...
* Good, because marp-to-editable-pptx の実証済み思想（[調査記録](../research/2026-07-07-pptx-approaches.md) の該当事実）を Slidev に適用できる。
...
関連調査と旧 facts/claims.yaml 由来の事実表は [docs/research/2026-07-07-pptx-approaches.md](../research/2026-07-07-pptx-approaches.md) を参照する。
